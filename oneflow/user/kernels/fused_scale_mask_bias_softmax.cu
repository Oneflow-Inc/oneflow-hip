...
void LaunchFusedSoftmaxForwardKernel(GPU(Stream_t) stream, T* out, const T* x, const T* mask,
...
template<typename T, typename ComputeType = typename cuda::softmax::DefaultComputeType<T>::type>
void LaunchSoftmaxBackwardKernel(GPU(Stream_t) stream, T* dx, const T* y, const T* dy, T scale,
