...
void launch_noncontiguous_binary_op_kernel(GPU(Stream_t) stream, const IndexType n_pack,
                                           Store& store, Load1& load1, Load2& load2) {
  int num_blocks = 1, block_size = cuda::elementwise::kBlockSize;
  GPU(Error_t) err = cuda::elementwise::GetNumBlocks(n_pack, &num_blocks);
  CHECK(err == GPU(Success));
...
void dispatchOp(GPU(Stream_t) stream, const std::string& op, const IndexType n_pack, Store& store,
...
void dispatchInplace(GPU(Stream_t) stream, const bool inplace, const std::string& op,
...
void dispatchIndexType(GPU(Stream_t) stream, const bool inplace, const std::string& op,
...
void dispatchPacksize(GPU(Stream_t) stream, const bool inplace, const std::string& op,
...
void launch_noncontiguous_binary_op_grad_kernel(GPU(Stream_t) stream, const IndexType n_pack,
...
template<int pack_size, typename IndexType, typename R, typename lhs, typename rhs, typename Loady,
         typename Load1, typename Load2>
void dispatchOpGrad(GPU(Stream_t) stream, const std::string& op, const IndexType& n_pack,
                    Loady& load_y, Load1& load1, Load2& load2) {
  if (op == "add")
...
template<int pack_size, typename IndexType, typename R, typename lhs, typename rhs>
void dispatchLoader(GPU(Stream_t) stream, const std::string& op, const int& ndims,
...
template<int pack_size, typename R, typename lhs, typename rhs>
void dispatchIndexTypeGrad(GPU(Stream_t) stream, const std::string& op, const int& ndims,
...
template<typename R, typename lhs, typename rhs>
void dispatchPacksizeGrad(GPU(Stream_t) stream, const std::string& op, const int& ndims,
