...
struct AddFunctor<hipComplex, U, Args...> {
  __device__ hipComplex operator()(hipComplex x0, U x1, Args... xs) const {
    hipComplex xn = AddFunctor<U, Args...>()(x1, xs...);
    return hipComplex{x0.x + xn.x, x0.y + xn.y};
  }
};

template<typename U, typename... Args>
struct AddFunctor<hipDoubleComplex, U, Args...> {
  __device__ hipDoubleComplex operator()(hipDoubleComplex x0, U x1, Args... xs) const {
    hipDoubleComplex xn = AddFunctor<U, Args...>()(x1, xs...);
    return hipDoubleComplex{x0.x + xn.x, x0.y + xn.y};
  }
};
